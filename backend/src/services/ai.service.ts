...
class AIService {
  ...
  async generateHints(request: GenerateHintsRequest): Promise<GeneratedHint[]> {
  ...
  async chat(request: ChatRequest): Promise<{ response: string; suggestions: string[] }> {
  ...
  async chatWithHistory(request: ChatWithHistoryRequest): Promise<{ response: string; suggestions: string[] }> {
...
`;
  ...
  private buildChatPrompt(message: string, context?: ChatRequest['context']): string {
    let contextSection = '';
...
`;
  ...
  private parseChatResponse(responseText: string): { response: string; suggestions: string[] } {
...
`;
  ...
  private parseHintsResponse(responseText: string, expectedCount: number): GeneratedHint[] {
    ...
    return hints.slice(0, expectedCount).map((h: any, index: number) => ({
