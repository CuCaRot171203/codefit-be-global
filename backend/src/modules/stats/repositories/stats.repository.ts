...
class StatsRepository {
  ...
  async getUserScoreBreakdown(userId: string): Promise<any> {
    ...
    for (const sub of lessonSubmissions) {
      ...
      let phase = courseData.phases.find((p: any) => p.phaseId === phaseId);
      if (!phase) {
      ...
      let lesson = phase.lessons.find((l: any) => l.lessonId === lessonId);
      if (!lesson) {
        lesson = {
          lessonId,
          lessonTitle,
  ...
  async getUserEnrolledCoursesWithProgress(userId: string): Promise<any> {
    const enrollments = await prisma.enrollment.findMany({
      where: { userId },
      include: {
        course: {
