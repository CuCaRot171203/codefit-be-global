 ...
 * @extends BaseService
 */
class ProjectService extends BaseService<typeof projectRepository> {
   ...
   * @returns Promise<Project[]> - Danh sách dự án
   */
  async getCourseProjects(courseId: string): Promise<any[]> {
   ...
   */
  async update(id: string, userId: string, dto: UpdateProjectDto): Promise<any> {
  ...
  async submitProject(params: {
    ...
    if (user?.email) {
      emailService.sendProjectSubmittedEmail(
        user.email,
        user.username,
        project.title,
    ...
    let certificateResult: { id: string; certificateUrl: string | null } | null = null;

    try {
      const certificate = await certificateService.generateForProjectSubmission(params.userId, courseId);
