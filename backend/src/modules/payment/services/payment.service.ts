...
if (sePayMerchantId && sePaySecretKey) {
  try {
    sePayClient = new SePayPgClient({
      env: sePayEnv as 'sandbox' | 'production',
...
class PaymentService {
   ...
   */
  async createPayment(userId: string, courseId: string, paymentMethod: 'qr' | 'direct') {
   ...
   */
  async createSePayCheckout(userId: string, courseId: string) {
   ...
   */
  private createMockCheckout(payment: any, course: any, orderCode: string) {
   ...
   */
  async activateWithCode(userId: string, code: string) {
    // Tìm activate code
   ...
   */
  async confirmSePayPayment(orderCode: string) {
    ...
    try {
      const user = await prisma.user.findUnique({ where: { id: payment.userId } }) as User | null;
      const course = await courseRepository.findById(payment.courseId) as any;
   ...
   */
  async createPayOSPaymentLink(userId: string, courseId: string) {
    const course = await courseRepository.findById(courseId);
    if (!course) {
      throw new Error('Course not found');
    }
   ...
   */
  async confirmPayOSPayment(orderCode: string, transactionId?: string) {
    const payment = await paymentRepository.findByPayosOrderId(orderCode);
   ...
   */
  async confirmPayOSPaymentById(paymentId: string, userId: string) {
    const payment = await paymentRepository.findById(paymentId);
    ...
    let payosStatus: string | undefined;
   ...
   */
  async getPayOSPaymentStatus(orderCode: string) {
    try {
   ...
   */
  async cancelPayOSPayment(paymentId: string, userId: string, reason?: string) {
