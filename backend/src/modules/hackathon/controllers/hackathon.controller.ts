...
class HackathonController extends BaseController {
   ...
   * @param next - Express NextFunction
   */
  getEnded = async (req: Request, res: Response, next: NextFunction) => {
    try {
   ...
   * @param next - Express NextFunction
   */
  getRegistered = async (req: Request, res: Response, next: NextFunction) => {
    try {
      const userId = req.user?.userId;
      if (!userId) {
        this.error(res, 'Unauthorized', 401);
  ...
  getProblems = async (req: Request, res: Response, next: NextFunction) => {
    try {
      const userId = req.user?.userId;
      if (!userId) { this.error(res, 'Unauthorized', 401); return; }
      const { id } = req.params;
