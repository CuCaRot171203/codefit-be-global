...
class LessonReviewService extends BaseService {
    constructor() {
    ...
    async approve(lessonId, adminId, feedback) {
        ...
        if (lessonRequest.lecture) {
            await emailService.sendLessonApprovedNotification(lessonRequest.lecture.email, lessonRequest.lecture.fullName || 'Giảng viên', lesson.title);
    ...
    async reject(lessonId, adminId, feedback) {
        ...
        // Notify lecture via email
        if (lessonRequest?.lecture) {
            await emailService.sendLessonRejectedNotification(lessonRequest.lecture.email, lessonRequest.lecture.fullName || 'Giảng viên', lesson.title, feedback);
            await notificationService.createNotification({
                userId: lessonRequest.lecture.id,
    ...
    async publish(lessonId, adminId) {
        ...
        for (const userId of enrolledUserIds) {
            ...
            if (user) {
                ...
                await emailService.sendNewLessonNotification(user.email, user.fullName || user.username, lesson.phase.course.title, lesson.title, lessonUrl);
            }
        }
    ...
    async autoEnrollUsers(courseId, courseTitle, lessonId, lessonTitle) {
        ...
        for (const user of usersToEnroll) {
            ...
            // Send email
            await emailService.sendNewLessonNotification(user.email, user.fullName || user.username, courseTitle, lessonTitle, lessonUrl);
        }
    }
