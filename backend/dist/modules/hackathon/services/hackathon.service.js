...
class HackathonService extends BaseService {
    ...
    async join(hackathonId, userId, teamName) {
        ...
        try {
            await notificationService.createHackathonJoinedNotification(userId, hackathon.title, hackathonId);
            ...
            if (user?.email) {
                await emailService.sendHackathonJoined(user.email, user.fullName || 'User', hackathon.title);
            }
        }
        catch (notifError) {
