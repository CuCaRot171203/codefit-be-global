 ...
 */
router.post('/', verifyToken, uploadMiddleware.single('file'), asyncHandler(async (req, res) => {
 ...
 * @param res - Response trả về thông tin file đã upload
 */
router.post('/image', verifyToken, uploadMiddleware.single('image'), asyncHandler(async (req, res) => {
    // Bước 1: Kiểm tra xem có file được upload không
    if (!req.file) {
        res.status(400).json({
            success: false,
 ...
 */
router.post('/images', verifyToken, uploadMiddleware.array('images', 10), asyncHandler(async (req, res) => {
    ...
    const uploadPromises = files.map((file) => uploadImage(file.buffer, file.originalname, folder));
