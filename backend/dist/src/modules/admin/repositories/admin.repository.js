...
class AdminRepository {
    ...
    async getPaymentStats() {
        const totalAmount = await prisma_1.default.payment.aggregate({
            where: { paymentStatus: 'completed' },
            _sum: { amount: true },
        });
        const pendingCount = await prisma_1.default.payment.count({
            where: { paymentStatus: 'pending' },
        });
        const completedCount = await prisma_1.default.payment.count({
            where: { paymentStatus: 'completed' },
        });
        const byMethod = await prisma_1.default.payment.groupBy({
    ...
    async createActivateCode(data) {
        return prisma_1.default.activateCode.create({ data });
    }
    async deleteActivateCode(id) {
        return prisma_1.default.activateCode.delete({ where: { id } });
    }
    // ============ Dashboard Stats ============
    async getDashboardStats() {
        const [totalUsers, totalCourses, totalEnrollments, totalLessons, totalMinitests, totalHackathons, paymentStats, usersByRole, recentUsers, activeHackathons, topSubmissions,] = await Promise.all([
            prisma_1.default.user.count(),
            prisma_1.default.course.count(),
            prisma_1.default.enrollment.count(),
            prisma_1.default.lesson.count(),
            prisma_1.default.minitest.count(),
            prisma_1.default.hackathon.count(),
            ...
            prisma_1.default.hackathon.findMany({
                take: 5,
            ...
            prisma_1.default.hackathonSubmission.findMany({
                take: 10,
                orderBy: { score: 'desc' },
                select: {
                    id: true,
    ...
    // ============ Phases (Chương học) ============
    async getAllPhases() {
        return prisma_1.default.phase.findMany({
            orderBy: { orderIndex: 'asc' },
            include: {
                course: { select: { id: true, title: true } },
                lessons: {
    ...
    async createPhase(data) {
        ...
        const maxOrder = await prisma_1.default.phase.aggregate({
            where: { courseId: data.courseId },
            _max: { orderIndex: true },
        });
        const orderIndex = (maxOrder._max.orderIndex ?? -1) + 1;
        return prisma_1.default.phase.create({
            data: {
                ...data,
    ...
    async updatePhase(id, data) {
        return prisma_1.default.phase.update({
            where: { id },
            data,
            include: {
                course: { select: { id: true, title: true } },
    ...
    async deletePhase(id) {
        return prisma_1.default.phase.delete({ where: { id } });
    }
    // ============ Lessons (Bài học) ============
    async getAllLessons() {
    ...
    async createLesson(data) {
        ...
        const maxOrder = await prisma_1.default.lesson.aggregate({
    ...
    async updateTestcase(id, data) {
        return prisma_1.default.testcase.update({
            where: { id },
            data,
        });
    }
    async deleteTestcase(id) {
        return prisma_1.default.testcase.delete({ where: { id } });
    }
    // ============ Minitests (Admin) ============
    ...
    async updateMinitest(id, data) {
        ...
        if (data.questionIds !== undefined) {
            await prisma_1.default.minitestQuestion.deleteMany({ where: { minitestId: id } });
    ...
    async deleteMinitest(id) {
        return prisma_1.default.minitest.delete({ where: { id } });
    }
    // ============ Problems (Admin) ============
    ...
    async deleteProblem(id) {
        return prisma_1.default.problem.delete({ where: { id } });
    }
    // ============ Lecture Course Assignment ============
    async getLectureCourses(lectureId) {
        return prisma_1.default.lectureCourse.findMany({
            where: { lectureId },
            include: {
                course: {
                    select: {
    ...
    async assignCourseToLecture(lectureId, courseId, assignedBy) {
        return prisma_1.default.lectureCourse.create({
            data: {
                lectureId,
                courseId,
                assignedBy
    ...
    async unassignCourseFromLecture(lectureId, courseId) {
        return prisma_1.default.lectureCourse.delete({
            where: {
                lectureId_courseId: {
                    lectureId,
                    courseId
    ...
    async isLectureAssignedToCourse(lectureId, courseId) {
        const result = await prisma_1.default.lectureCourse.findUnique({
            where: {
                lectureId_courseId: {
                    lectureId,
                    courseId
    ...
    async getLecturesByCourse(courseId) {
        const lectureCourses = await prisma_1.default.lectureCourse.findMany({
            where: { courseId },
            include: {
                lecture: {
                    select: {
    ...
    async getInstructorDetail(lectureId) {
        try {
            ...
            const lessonRequests = await prisma_1.default.lessonRequest.findMany({
                where: { lectureId },
                include: {
                    lesson: {
                        include: {
            ...
            const minitestSubmissions = await prisma_1.default.minitestSubmission.findMany({
            ...
            const hackathonParticipations = await prisma_1.default.hackathonParticipant.findMany({
                where: { userId: lectureId },
                include: {
                    hackathon: true,
                },
            ...
            const hackathonSubmissions = await prisma_1.default.hackathonSubmission.findMany({
                where: { userId: lectureId },
                include: {
                    hackathon: {
                        select: {
            ...
            const courseProgress = lectureCourses.map((lc) => {
                const courseLessonRequests = lessonRequests.filter((lr) => lr.lesson?.phase?.course?.id === lc.course.id);
                const completed = courseLessonRequests.filter((lr) => lr.status === 'SUBMITTED' || lr.status === 'APPROVED').length;
            ...
            return {
                ...
                stats: {
                    ...
                    avgMinitestScore: minitestSubmissions.length > 0
                        ? Math.round(minitestSubmissions.reduce((acc, ms) => acc + ms.score, 0) / minitestSubmissions.length)
                        : 0,
    ...
    async getAllHackathons() {
        return prisma_1.default.hackathon.findMany({
            include: {
                course: {
    ...
    async createHackathon(data) {
        ...
        return prisma_1.default.hackathon.create({
            data: {
                ...hackathonData,
    ...
    async updateHackathon(id, data) {
        return prisma_1.default.hackathon.update({
            where: { id },
            data: {
                title: data.title,
                description: data.description,
    ...
    async deleteHackathon(id) {
        return prisma_1.default.hackathon.delete({ where: { id } });
    }
    ...
    async approveProjectSubmission(submissionId) {
        return prisma_1.default.projectSubmission.update({
    ...
    async getLessonRequestById(id) {
        return prisma_1.default.lessonRequest.findUnique({
            where: { id },
            include: {
    ...
    async getAllLessonRequests() {
        return prisma_1.default.lessonRequest.findMany({
            include: {
                lecture: { select: { id: true, fullName: true, email: true } },
                lesson: {
                    include: {
    ...
    async deleteLessonRequest(id) {
        return prisma_1.default.lessonRequest.delete({ where: { id } });
    }
    async approveLessonRequest(id) {
        return prisma_1.default.lessonRequest.update({
            where: { id },
            data: {
                status: 'APPROVED',
                reviewedAt: new Date(),
    ...
    async getMinitestStats() {
        ...
        const minitests = await prisma_1.default.minitest.findMany({
            include: {
                phase: {
                    include: {
                        course: { select: { id: true, title: true } }
