...
class ScoringService {
    ...
    async calculateScore(lessonId, code, language, hintsUsed = 0, timeUsed = null) {
        ...
        // Get scoring config
        const scoringConfig = await prisma_1.default.scoringConfig.findUnique({
            where: { lessonId },
        });
        const config = {
    ...
    async saveSubmission(lessonId, userId, code, language, hintsUsed, timeUsed) {
        ...
        const submission = await prisma_1.default.lessonSubmission.create({
            data: {
                lessonId,
                userId,
                code,
    ...
    async getUserSubmissions(lessonId, userId) {
        return prisma_1.default.lessonSubmission.findMany({
