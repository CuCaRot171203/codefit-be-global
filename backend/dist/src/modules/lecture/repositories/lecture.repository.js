...
class LectureRepository {
    ...
    async findMinitestsByLectureId(lectureId) {
        ...
        const submissionsByMinitest = new Map();
        ...
        return minitests.map(minitest => {
            ...
            const avgScore = totalAttempts > 0
                ? Math.round(minitestSubmissions.reduce((sum, s) => sum + s.score, 0) / totalAttempts)
                : 0;
            return {
                id: minitest.id,
    ...
    async findHackathonsByLectureId(lectureId) {
        ...
        return hackathons.map(hackathon => {
            ...
            const avgScore = hackathon.submissions.length > 0
                ? Math.round(hackathon.submissions.reduce((sum, s) => sum + s.score, 0) / hackathon.submissions.length)
                : 0;
            return {
                id: hackathon.id,
...
exports.default = new LectureRepository();
