...
class AIController {
    ...
    generateHints = async (req, res, next) => {
        try {
            ...
            const lessonContent = await prisma_1.default.lessonContent.findUnique({
                where: { lessonId },
            });
            ...
            const hints = await ai_service_1.default.generateHints({
            ...
            // Log the AI request
            await prisma_1.default.aIRequest.create({
                data: {
    ...
    chat = async (req, res, next) => {
        try {
            ...
            const result = await ai_service_1.default.chat({ message, context: fullContext });
            // Log the AI request
            await prisma_1.default.aIRequest.create({
                data: {
                    userId: req.user?.userId || 'unknown',
    ...
    sendMessage = async (req, res) => {
        try {
            ...
            // Create user message
            const userMsg = await prisma_1.default.conversationMessage.create({
                data: {
                    conversationId: id,
                    role: 'user',
                    content: message,
            ...
            const result = await ai_service_1.default.chatWithHistory({
                message,
                history: historyMessages,
            });
            // Save AI response
            const assistantMsg = await prisma_1.default.conversationMessage.create({
                data: {
                    conversationId: id,
                    role: 'assistant',
